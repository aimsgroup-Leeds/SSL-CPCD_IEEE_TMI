...
def get_argparser():
    ...
    # Datset Options
    parser.add_argument("--data_root", type=str, default='./kvasir/',
    ...
    parser.add_argument("--backbone", type=str, default='resnet50',
                        choices=['vgg19',  'resnet34' , 'resnet50',
                                 'resnet101', 'densenet121', 'none','resnet50_cbam'], help='model name')
...
def get_dataset(opts):
    ...
    train_dst = polyGenSeg(root=opts.data_root, image_set='train_kvasir', download=opts.download, transform=train_transform)
    val_dst = polyGenSeg(root=opts.data_root, 
                                  image_set='val_kvasir', download=False, transform=val_transform)
...
def validate(opts, model, loader, device, metrics, ret_samples_ids=None):
    ...
    metrics.reset()
    ret_samples = []
    directoryName = 'results_%s_%s_%s'%(opts.dataType, opts.model,opts.backbone)
...
def main():
    ...
    if opts.model =='SSL_CPCD':
        from unet.unet import Unet
        
        model = Unet(backbone_name=opts.backbone, pretrained=False, classes = opts.num_classes)
    ...
    while True: 
        ...
        for (images, labels) in train_loader:
            ...
            if (cur_itrs) % opts.val_interval == 0:
                save_ckpt('checkpoints/latest_%s_%s_os%d_%s_%s.pth' %
                ...
                if val_score['Mean IoU'] > best_score:  # save best model
                    ...
                    save_ckpt('checkpoints/best_%s_%s_os%d_%s_%s.pth' %
                              (opts.model, opts.dataset,opts.output_stride, opts.dataType, opts.backbone))
